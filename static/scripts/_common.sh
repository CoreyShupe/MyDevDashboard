...
# Resolve repo root regardless of where the script is invoked from. These scripts live at
# static/scripts/, so the repo root is two levels up.
...
# The compose file lives under static/docker/ (not the repo root). All compose invocations go
# through the `compose()` helper below so the `-f` flag stays in one place. The named external
# volume + pinned project name mean the file's LOCATION does not affect which container/volume
# is used — moving the file cannot orphan the owner's data.
COMPOSE_FILE="$REPO_ROOT/static/docker/docker-compose.yml"
...
# Single entry point for docker compose, always pointing at the moved compose file.
...
start_db() {
  ...
  echo "error: postgres did not become healthy in time. Check logs with: docker compose -f $COMPOSE_FILE logs postgres" >&2
  return 1
}
